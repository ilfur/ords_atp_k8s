...
function setupOrds() {
  ...
  ORACLE_USER=${ORACLE_USER:-"ORDS_PUBLIC_USER"}
  ...
  mkdir $ORDS_HOME/config/$CONTEXT_ROOT/conf
  cp $ORDS_HOME/apex_pu.xml $ORDS_HOME/config/$CONTEXT_ROOT/conf

  sed -i -e "s|###ORACLE_SERVICE###|$ORACLE_SERVICE|g" $ORDS_HOME/config/$CONTEXT_ROOT/conf/apex_pu.xml
  sed -i -e "s|###ORACLE_USER###|$ORACLE_USER|g" $ORDS_HOME/config/$CONTEXT_ROOT/conf/apex_pu.xml
  sed -i -e "s|###ORDS_PWD###|$ORDS_PWD|g" $ORDS_HOME/config/$CONTEXT_ROOT/conf/apex_pu.xml

  echo -n "<entry key=\"db.wallet.zip\"><![CDATA[" >> $ORDS_HOME/config/$CONTEXT_ROOT/conf/apex_pu.xml
  cat $ORDS_HOME/wallet.b64 >> $ORDS_HOME/config/$CONTEXT_ROOT/conf/apex_pu.xml
  echo -n "]]></entry></properties>" >> $ORDS_HOME/config/$CONTEXT_ROOT/conf/apex_pu.xml
  ...
  sed -i -e "s|###ORACLE_USER###|$ORACLE_USER|g" $ORDS_HOME/params/ords_params.properties
